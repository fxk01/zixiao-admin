...
define([
  ...
  var re_answerPage = {
    init: function () {
      ...
      var self = this;
      var re_anscontent = CKEDITOR.replace("re_anscontent");
      $.AMUI.progress.start();
      ...
      $('#body').on('click', '.edit', function () {
        self.editGoods($(this),re_anscontent);
      });
...
//    操作按钮  '#editFormUp'
    editGoods: function(self,re_anscontent) {
      ...
      $("#editFormUp").find('textarea[name="ask_content"]').val(self.attr('data-ask_content'));
        ...
        re_anscontent.setData(self.attr('data-content'));
    ...
    auditQaFn:function (self,audstatus,comment) {
      singleMode.ajaxFun(singleMode.url + 'audit/cpnflow', { aid:self.attr('data-aid'),cpnid:self.attr('data-cpnid'),fstatus: audstatus,comment:comment}, 'get', function(json) {
        if(json['ok'] === true && json['code']===1000) {
          $('#my-popup').modal('close');   //关闭操作弹框；
          $('#popupCancel').modal('close');
          re_answerPage.conTacTinFo();
          alert('成功！');
        } else {
          alert('失败！');
    ...
    conTacTinFo: function () {
      ...
      singleMode.ajaxFun(singleMode.url + 'todo/answer/cpnflow/data/all', {page: 1, pagesize: pagesize,fstatus:fstatus}, 'get', function(json) {
        if(json['ok'] === true && json.data) {
          $('#page4Curr').children().remove();
          if(json['totals'] > pagesize) {
            $('#page4Curr').append(' <div class="am-fr" id="page4">\n' +
          ...
          var thisDate = function(curr) {
            if(curr > 1) {
              ...
              jsonList1.forEach(function (item) {
                ...
                if(item['answer']){
                    if(item['answer']['content'].indexOf("'")){
                        item['answer']['re_content']=item['answer']['content'].replace(pingFang,'"');
                    }
                  item['answer']['re_content']=item['answer']['content'].replace(reg5,' ')
                }
              });
              jsonList=jsonList1;
              var interText = doT.template($('#calendarTp4').text());
    ...
    postOrder2: function(fstatus, curr, numS) {
      singleMode.ajaxFun(singleMode.url + 'todo/answer/cpnflow/data/all', { page: curr, pagesize: numS ,fstatus:fstatus}, 'get', function(json) {
        if(json['ok'] === true && json.data) {
